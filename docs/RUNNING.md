...
4. Effective-mass fit — **always runs**; `En_<type>_<tag>.pdf` only if `plot_meff=True`
5. `Zn_meson_<tag>.pdf` if `plot_meff=True` (Zn is meson-only; skipped internally in tetraquark mode)
