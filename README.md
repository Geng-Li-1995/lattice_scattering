...
> **Data availability:** Raw lattice correlators (`.npy`, ~80 MB) and jackknife/bootstrap resampled files are **not included** in this repository due to file size and data-access constraints. The full analysis **code** and **result figures** are provided; clone the repo, place your own data under `data/<system>/raw/` (and run `run_resample.py` for scattering), then execute `main.py` to reproduce the workflow. See [Data Layout](#data-layout) and [docs/RUNNING.md](docs/RUNNING.md).
...
│   └── plot_scattering.py
├── docs/
│   └── figures/             # PNG previews for README (from result PDFs)
└── result/<system>/         # Output PDF figures (tracked in Git)
```
...
**Not provided in this repository:** raw correlators (`data/<system>/raw/*.npy`) and resampled energies (`data/<system>/resampled/*.npy`). You must supply these locally to run the pipeline from scratch.

**Provided in this repository:** analysis source code, configuration, documentation, result PDFs under `result/Tcccc6600/`, and PNG previews under `docs/figures/`.

### Raw correlators (`data/<system>/raw/`) — local only
...
### Resampled outputs (`data/<system>/resampled/`) — local only
...
Representative outputs for **\(L = 12\)** (`L12M420_EV170`). Full-resolution PDFs for both \(L = 12\) and \(L = 16\) are in [`result/Tcccc6600/`](result/Tcccc6600/).
...
- Scattering channel indices and fit-momentum subsets are configured in `input/Tcccc6600_input.py` (`ch_meson_a`, `ch_meson_b`, `ch_tetra`, `fit_mom_by_ns`).

---

## Version Control

This project uses Git. Raw/resampled `.npy` data are excluded via `.gitignore`; **result PDFs** and **README preview PNGs** are tracked for portfolio display.

```bash
git clone https://github.com/Geng-Li-1995/lattice_scattering.git
cd lattice_scattering
# Place correlators under data/Tcccc6600/raw/ (not included in repo)
pip install -r requirements.txt
python main.py
```

After cloning, obtain or generate lattice correlators locally — they are **not** stored in the repository (~80 MB). See [docs/RUNNING.md](docs/RUNNING.md) for the expected file layout.
